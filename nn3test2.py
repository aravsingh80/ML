...
def error2(perc, expected, w): 
    ...
    for x in range(len(perc) - 1, 0, -1):
        if x == len(perc) - 1:
            ...
            errors = []
            for x in range(len(perc[len(perc) - 2])): errors.append((expected - perc[len(perc) - 1][x]) * w[-1][x] * sigmoidPrime(perc[len(perc) - 2][x]))
            allErrors.append(errors)
        else:
            errors = []
            count = 0
            for y in range(len(perc[x])):
                ...
                for z in range(len(perc[x + 1])):
                    error = (w[x][count] * allErrors[len(allErrors) - 1][z]) * (sigmoidPrime(perc[x][y]))
                    totalError += error
                    count += 1
...
while epochs < 600:
    ...
    for b, exp in trainingSet:
        output = perceptron(sigmoid, b, mainWList)
        mainWList, currError = backProp(mainBList, output, mainWList, a, exp)
        endOutput = perceptron(sigmoid, b, mainWList)[-1][0]
